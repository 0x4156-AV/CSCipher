...
def beenClicked():
    ...
    if cipherType == "Caesar":
    	text = Text(app, height=1, font=("Helvetica", 32))
	text.insert(INSERT, "Caesar Cipher")
	text.config(state=DISABLED)
	text.grid(row=3,columnspan=2, padx=5)
	text = Text(app, height=1, font=("Helvetica", 16))
	text.insert(INSERT, "Explination")
	text.config(state=DISABLED)
	text.grid(row=3,columnspan=2, padx=5)
    	labelText = StringVar(None)
	textEntry = Entry(app, width=57, justify=CENTER, textvariable=labelText).grid(row=6, columnspan=4, padx=5, pady=5)
	submitButton = Button(app, text="Start", width=10, command=callback).grid(row=6, column=4, pady=5)
